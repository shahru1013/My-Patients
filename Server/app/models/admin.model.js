...
Admin.insertAPrescription = (data, pat_id, result) =>{
  console.log('dataaa press ',JSON.stringify(data?.allsymp), data);
 ...
 //SELECT * FROM prescriptions WHERE id = SCOPE_IDENTITY()
  sql.query(`INSERT INTO prescriptions (id, patient_id, symptoms, tests, advice, prescription, date, doctor_id, fees, payment_option, appointment_time, systolic, diastolic, temparature, heart_rate) VALUES ('', '${pat_id}', '${JSON.stringify(data?.allsymp) || []}', '${JSON.stringify(data?.allTests) || []}', '${JSON.stringify(data?.allAdvices) || []}', '${JSON.stringify(data?.allMedicines) || []}', '${moment(new Date()).format("DD/MM/YYYY")}', '', '', '', '${new Date()}', '${data?.systolic || 0}', '${data?.diastolic || 0}', '${data?.bodyTemp || 0}', '${data?.heartRate || 0}');`, (err, res)=>{
